...
//defined in sys.c
extern int restart_or_shutdown;
#endif
#endif
...
//mayu close,bootanimal not show 
#ifdef CONFIG_ZTEMT_LCD_V5
	.cont_splash_enabled = 0x1, //0x1,
#else
	.cont_splash_enabled = 0, //0x1,
#endif
#endif
...
static int mipi_dsi_zte_panel_power(int on)
{
	...
	if (on) {
		...
		if (SHARP_R69431 == mini2_get_lcd_type()) {
			...
			if (is_firston) {
			// pls keep the annotation and do not restart lcd when video and command mode for otm1282a
