...
static uint8_t is_keyboard_ready( void )
{
    ...
    if (is_mdt_dev_disabled(DEV_TYPE_KEYBOARD)) return 0;
...
static uint8_t is_mouse_ready( void )
{
    ...
    if (is_mdt_dev_disabled(DEV_TYPE_MOUSE)) return 0;
...
static uint8_t is_touch_ready( void )
{
    ...
    if (is_mdt_dev_disabled(DEV_TYPE_TOUCH)) return 0;
...
unsigned char 	mdt_generate_event_touchscreen(struct mdt_cursor_other_t *touchPacket, uint8_t submitEvent)
{
    ...
    if (is_touch_ready() == 0) return 0xFF;
		...
		if (keycodes_new[i] > 3 && memscan(keycodes_old + 1, keycodes_new[i], 6) == keycodes_old + 7) {
			if (usb_kbd_keycode[keycodes_new[i]]) {
				switch(usb_kbd_keycode[keycodes_new[i]]) {
					#ifdef PHOENIX_BLADE					
					case KEY_F1:  mdt.phoenix_blade_state = MDT_PB_SIMULATED_MOUSE_0; 	is_mouse_ready(); 	break;
					case KEY_F2:  mdt.phoenix_blade_state = MDT_PB_SIMULATED_MOUSE_90; 	is_mouse_ready(); 	break;
					case KEY_F3:  mdt.phoenix_blade_state = MDT_PB_SIMULATED_MOUSE_180;	is_mouse_ready(); 	break;
					case KEY_F4:  mdt.phoenix_blade_state = MDT_PB_SIMULATED_MOUSE_270; 	is_mouse_ready(); 	break;
					case KEY_F12: mdt.phoenix_blade_state = MDT_PB_NATIVE_TOUCH_SCREEN;	mouse_deregister(1);	break;
					case KEY_LEFTCTRL:  
				...
				SI_MDT_DEBUG_PRINT((KERN_ERR "Unknown key (scancode %#x) pressed.\n", keycodes_new[i]));
			}
		}
     }
