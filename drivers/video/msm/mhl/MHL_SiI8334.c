...
#ifdef ZTE_MHL_INTERRUPT_FALLINGEDGE
static irqreturn_t Sii8334_mhl_interrupt_t(int irq, void *dev_id)
{
	SiiMhlTxDeviceIsr();
	return IRQ_HANDLED;
}
			...
			const struct i2c_device_id *dev_id)
{
		...
		&& sii8334_PAGE_CBUS != NULL)
	{
		...
		ret = request_threaded_irq(sii8334_PAGE_TPI->irq,NULL,&Sii8334_mhl_interrupt_t,IRQF_TRIGGER_LOW | IRQF_ONESHOT,sii8334_PAGE_TPI->name, sii8334_PAGE_TPI);
...
{
//+++ duguowei,do not disable_irq,so no need to enable_irq agian;
	//enable_irq(sii8334_PAGE_TPI->irq);
//--- duguowei,do not disable_irq,so no need to enable_irq agian;
}
