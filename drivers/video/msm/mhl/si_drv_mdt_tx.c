...
static void mdt_cbus_read_block(unsigned char reg_offset, unsigned char byte_count, unsigned char *data)
{
	...
	if (ret < 0)
	{
		...
		g_mdt.i2c_cbus_client->timing=400;
        //set spedd, panjie
...
static uint8_t sii8338_parse_received_burst_for_mdt(union mdt_event_t *mdt_packet,SiiReg_t scratchpad_offset)
{
...
/*
        printk("MHL data: %x %x %x %x %x %x %x %x %x\n",
		mdt_packet->bytes[0],
		mdt_packet->bytes[1],
		mdt_packet->bytes[2],
		mdt_packet->bytes[3],
...
static void sii8338_msc_req_for_mdt(uint8_t req_type,
		uint8_t offset, uint8_t first_data)
{
...
uint8_t sii8338_irq_for_mdt(enum mdt_state *mdt_state) 
{
	...
	switch (*mdt_state) {
		case  WAIT_FOR_REQ_WRT:
			...
			{	MStatus=0;	
				sii8338_msc_req_for_mdt(0x01 << 3,		
					OFFSET_SET_INT, GRT_WRT); 				
		...
		case WAIT_FOR_WRITE_BURST_COMPLETE:
			...
			{	//HalReleaseIsrLock();			
				...
				if (mdt_packet.header.isNotLast)				
					...
					//SiiRegWrite(TX_PAGE_CBUS | 0x00A0,	MHL_INT_DSCR_CHG); 
				ret = MDT_EVENT_HANDLED;
				*mdt_state = WAIT_FOR_REQ_WRT;				
			}
			else if(intr & BIT2)
				{
				printk (KERN_ERR "intr & BIT2; ;;;;;;;;;;;;;;;;;;;;;;\n");
					ret = MDT_EVENT_HANDLED;
				*mdt_state = WAIT_FOR_REQ_WRT;	
				}
			/*
			else
			{//printk (KERN_ERR "MStatus: %x\n",MStatus);
			//SiiRegWrite(TX_PAGE_CBUS | 0x001E, BIT2);
			MStatus++;
			#if 1
				if(MStatus>0x5){
					MStatus=0;	
					ret = MDT_EVENT_FAILED;
					*mdt_state = WAIT_FOR_REQ_WRT;	
					//printk (KERN_ERR "MStatus++>0x30; ;;;;;;;;;;;;;;;;;;;;;;\n");
					}
				#endif
			}
			*/
...
void mdt_init(void) {
	if (g_mdt_is_ready != 0 ) return;

	if (sii8334_PAGE_TPI == 0) return;

	g_mdt.i2c_cbus_client = sii8334_PAGE_TPI;
...
{
	if (g_mdt_is_ready 	== 0) return;
	if (g_mdt_class 	== 0) return;
	if (g_mdt_dev		== 0) return;
