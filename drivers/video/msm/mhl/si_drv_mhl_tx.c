...
//extern void msm_hsusb_vbus_power_mhl(bool on);
...
/*
static	int	Int4Isr_disconnect( void )
{
	uint8_t int4Status;
	int4Status = SiiRegRead(TX_PAGE_3 | 0x0021);
	TX_DEBUG_PRINT(("INT4 Status = %02X.........................\n", (int) int4Status));
    	if (int4Status & BIT5)
    	{	TX_DEBUG_PRINT(("INT4 Status = %02X........................Int4Isr_disconnect.\n", (int) int4Status));
    		SiiRegWrite(TX_PAGE_3 | 0x0021, BIT5);	
...
void SiiMhlTxDeviceIsr (void)
{
	...
	unsigned char mdt_retry;
	//
	// Look at discovery interrupts if not yet connected.
	//

	//i=0;
	//TX_DEBUG_PRINT(("Drv: g_state_for_mdt=%d@@@@@@@@@@\n",g_state_for_mdt));
	...
	{//TX_DEBUG_PRINT(("Drv: g_state_for_mdt=%d!!!!!!!!!!!\n",g_state_for_mdt));
					...
					for (mdt_retry = 1; (mdt_retry < 30) && (g_state_for_mdt != IDLE); mdt_retry++) {
						//TX_DEBUG_PRINT(("Drv: g_state_for_mdt=%d,mdt_retry=%d\n",g_state_for_mdt,mdt_retry));
						
						// If MDT state machine caught an expected event; reset retry counter and run again.
						if (sii8338_irq_for_mdt(&g_state_for_mdt) == MDT_EVENT_HANDLED){
							mdt_retry = 0;
							//break;
							}
						//if(Int4Isr_disconnect()==I2C_INACCESSIBLE)
						//	return;
					}
       ...
       if( (POWER_STATE_D0_MHL == fwPowerState)&&(g_state_for_mdt==IDLE) )
		{
		...
		intMStatus = (SiiRegRead(TX_PAGE_L0| 0x0070))&0x01;
		/*if(0xFF == intMStatus)
		{
			intMStatus = 0;
