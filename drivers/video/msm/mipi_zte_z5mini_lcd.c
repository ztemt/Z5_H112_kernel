...
static void mipi_z5mini_set_backlight(struct msm_fb_data_type *mfd)
{
	...
	if (TIANMA_OTM1282A == mini2_get_lcd_type()){
		mipi_tianma_set_backlight(mfd);
	} else {
...
static int mipi_z5mini_lcd_on(struct platform_device *pdev)
{
	...
	if (SHARP_R69431 == mini2_get_lcd_type()) {
    	...
    	ret=mipi_r69431_lcd_on_cmd(pdev);
#else
		ret=mipi_r69431_lcd_on(pdev);
#endif
	} else if (TIANMA_OTM1282A == mini2_get_lcd_type()){
#ifdef CONFIG_ZTEMT_LCD_CMD_MODE
		ret=mipi_tianma_lcd_on_cmdmode(pdev);
#else
    	ret=mipi_tianma_lcd_on(pdev);
#endif 	
  	} else {
   		// FIXMI:NO_LCD. to normally enter desktop, use sharp r69431 in command mode
		ret=mipi_r69431_lcd_on(pdev);
	}
#endif
	return ret;
...
#if defined( CONFIG_ZTEMT_LCD_Z5MINI2_SIZE_4P7)
  #ifdef CONFIG_ZTEMT_LCD_CMD_MODE
    if (SHARP_R69431 == mini2_get_lcd_type() || TIANMA_OTM1282A == mini2_get_lcd_type()) {
       mipi_cmd_sharp_r69431_panel_info(&z5mini_panel_data.panel_info);
    } else {
