...
#endif
#ifdef VIBRATOR_MUTI_FEATURE
    __ATTR(usermode,0644, attr_read_vibrator_effecte_mode, attr_set_vibrator_effecte_mode),
...
//    drv260x_auto_cal();
	...
	vibdata.dev.name = "vibrator";
	vibdata.dev.get_time = vibrator_get_time;
	vibdata.dev.enable = vibrator_enable;

    if (timed_output_dev_register(&vibdata.dev) < 0)
    {
    ...
    status = create_sysfs_interfaces(vibdata.dev.dev);
	if (status < 0) 
    {
		dev_err(&client->dev,
