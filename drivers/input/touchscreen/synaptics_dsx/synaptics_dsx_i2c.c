...
#define ZTEMT_SYNAPTICS_PALM_SLEEP  1   //Added by luochangyang, for palm sleep  2013/11/30
...
static int synaptics_rmi4_sensor_report(struct synaptics_rmi4_data *rmi4_data)
{
    ...
    if (!rmi4_data->sensor_sleep) {
        ...
        if (extended_status & 0x02) {
			dev_dbg(&(rmi4_data->input_dev->dev), "%s: Have a palm  %d_%d\n",	
		...
		const struct i2c_device_id *dev_id)
{
    ...
    __set_bit(KEY_F10, rmi4_data->input_dev->keybit);
    /***ZTEMT END***/
